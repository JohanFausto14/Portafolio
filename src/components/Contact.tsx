...
const Contact = () => {
    const { t } = useLanguage();
    return (
        <section id="contact" className="py-12 bg-slate-900 min-h-screen flex items-center">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    ...
                    >
                        ...
                        <div className="space-y-6">
                            <a href={`mailto:${t.personal?.email}`} className="flex items-center gap-4 bg-slate-800/50 p-6 rounded-2xl border border-slate-700/50 hover:border-cyan-500/30 transition-colors group cursor-pointer">
                                ...
                                <div>
                                    ...
                                    <p className="text-gray-400 group-hover:text-cyan-400 transition-colors">{t.personal?.email}</p>
                                </div>
                            </a>
                            <a href={`tel:${t.personal?.phone?.replace(/[^\d+]/g, '')}`} className="flex items-center gap-4 bg-slate-800/50 p-6 rounded-2xl border border-slate-700/50 hover:border-cyan-500/30 transition-colors group cursor-pointer">
                                <div className="p-3 bg-slate-800 rounded-lg text-cyan-400 group-hover:scale-110 transition-transform">
                                    <Phone className="w-6 h-6" />
                                </div>
                                <div>
                                    <h4 className="text-lg font-medium text-white">{t.contact.phone}</h4>
                                    <p className="text-gray-400 group-hover:text-cyan-400 transition-colors">{t.personal?.phone}</p>
                                </div>
                            </a>
                    ...
                    >
                        <form className="space-y-6">
                            ...
                            <div className="pt-2 border-t border-slate-800">
                                <p className="text-sm text-gray-400 mt-4 mb-3 text-center">Enviar usando / Send using:</p>
                                <div className="grid grid-cols-3 gap-3">
                                    <button
                                        ...
                                        onClick={() => {
                                            ...
                                            if(!name || !message) { alert('Por favor llena tu nombre y mensaje.'); return; }
                                            
                                            const email = t.personal?.email || 'johanalvarado296@gmail.com';
